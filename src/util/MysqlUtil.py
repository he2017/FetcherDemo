...
def getConn():
    conn = None
    try:
        conn = MySQLdb.connect(host='localhost', #数据库地址 
                               user='root', #数据库账户名称
                               passwd='123456', #账户密码
                               db='fetcher_data',  #数据库名称
                               port=3306, #端口
                               charset="utf8") #编码方式
    except MySQLdb.Error,e: #将数据库处理异常信息打印到控制台方便调试
...
def saveHtml(urlSum, url, html):
    try:
        ...
        sql = "insert into html_data (url_sum, url, html) values('%s', '%s', '%s')"%(urlSum, url, MySQLdb.escape_string(html))
        cur.execute(sql)
...
def saveNewsContent(url, urlSum, title, author, pubTime, content):
    try:
        conn = getConn() #获取数据库连接
        cur = conn.cursor() #打开游标
        sql = "insert into news_content (url, url_sum, title, author, pub_time, content) values('%s', '%s', '%s', '%s', '%s', '%s')"%(url, urlSum, title, author, pubTime, MySQLdb.escape_string(content))
        cur.execute(sql)
        conn.commit() #提交事务，注意少了这一步插入数据将会失败
        closeConn(conn, cur) #关闭资源
        return True
